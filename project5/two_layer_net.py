...
import torch
import random
from p5_utils import sample_batch
...
def nn_forward_backward(
    ...
    # First compute the softmax of the last layer's output, storing it in the  #
    # variable `softmax` (compute this using elementary PyTorch operations).   #
    # Using this, compute the cross-entropy loss (CEL) and then compute the    #
    # total loss, which should include both CEL and L2 regularization for W1   #
    # and W2. Store the result in the variable `loss``, which should be a      #
    # scalar. When you implement the regularization over W, please DO NOT      #
    # multiply the regularization term by 1/2 (no coefficient). If you are not #
    ...
    grads = {}
    ###########################################################################
    # The backward pass computes the derivatives of the weights and biases,   #
    # storing the results in the grads dictionary.                            #
    # For example, grads['W1'] stores the gradient on W1, and is a tensor of  #
    # tensor of same size.                                                    #
    ...
    _,H=h1.shape
    R_bar=reg
    z2_bar=softmax.detach().clone()-torch.nn.functional.one_hot(y,num_classes=C)
    w2_bar=1/N*torch.mm(torch.transpose(h1, 0, 1),z2_bar)+R_bar*2*W2
    b2_bar=1/N*torch.sum(z2_bar,0)
    h_bar=z2_bar.mm(torch.transpose(W2,0,1))
    z1_bar=h_bar.detach().clone()
    z1_bar[h1==0]=0
    w1_bar=1/N*torch.transpose(X, 0, 1).mm(z1_bar)+R_bar*2*W1
    b1_bar=1/N*torch.sum(z1_bar,0)
    grads['W1']=w1_bar
    grads['W2']=w2_bar
    grads['b1']=b1_bar
    grads['b2']=b2_bar
